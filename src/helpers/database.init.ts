...
export async function initDatabase(options: AuthOptions) {
  switch (options.dbType) {
    case "mongo": {
      const { mongoUri, userCollectionName, magicLinkCollectionName } =
        options.dbOptions;
      ...
      if (!userCollectionName)
        throw new Error(
          "userCollectionName is required in dbOptions for MongoDB",
        );
    ...
    case "postgres": {
      const { postgresUrl, userTableName, magicLinkTableName } =
        options.dbOptions;
      ...
      if (!userTableName)
        throw new Error(
          "userTableName is required in dbOptions for PostgreSQL",
        );
    ...
    default:
      authLog(
        "error",
        `Unsupported dbType identified. Expected "mongo" or "postgres" `,
      );
      throw new Error(
        `Unsupported dbType identified. Expected "mongo" or "postgres"`,
      );
