 ...
 * Validates required tables, columns, and foreign key relationships
 * Magic link table is optional, inferred from authTypes
 */
export async function initPostgres(
  ...
  for (const col of requiredUserColumns) {
    if (!existingUserColumns.includes(col)) {
      throw new Error(
        `User table '${userTableName}' missing required column '${col}'`,
      );
    }
  }
  ...
  if (authTypes?.includes("magic-link")) {
    ...
    const magicTableCheck = await pool.query(
      `SELECT to_regclass($1) AS table_exists`,
      [magicTable],
    );
    ...
    // Validate required magic link columns
    const magicColumnsRes = await pool.query(
      `SELECT column_name
       FROM information_schema.columns
       WHERE table_name = $1`,
      [magicTable],
    ...
    for (const col of requiredMagicColumns) {
      if (!existingMagicColumns.includes(col)) {
        throw new Error(
          `Magic link table '${magicTable}' missing required column '${col}'`,
        );
      }
    }
    ...
    const validFK = fkCheck.rows.some(
      (r) =>
        r.referenced_table === userTableName && r.referenced_column === "id",
    );
